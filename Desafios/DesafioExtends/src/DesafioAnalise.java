...
public class DesafioAnalise {
  ...
  public static void main(String[] args) throws ParseException {
    ...
    List < String > analisesOK = sistemaAcionistas.obterAnalisesDesempenho(dataInicial, dataFinal);

    for (String analise: analisesOK) {
  ...
  public List < String > obterAnalisesDesempenho(String dataInicialStr, String dataFinalStr) throws ParseException {
   ...
   //TODO: Implemente o filtro das análises dentro do período especificado. Dica: Crie uma lista para armazenar as análises filtradas e use um loop for para filtrar as análises.
    ...
    for (Analise analise : analises) {
        if ((analise.getData().after(dataInicial) && analise.getData().before(dataFinal)) || analise.getData().equals(dataFinal) || analise.getData().equals(dataInicial))
            filtradas.add(analise.getDescricao());
    }
